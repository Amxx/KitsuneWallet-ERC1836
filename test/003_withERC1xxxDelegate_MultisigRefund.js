...
contract('ERC1xxxDelegate_MultisigRefund', async (accounts) => {
	...
	it ("Create proxy", async () => {
		Proxy = await ERC1xxx.new(
			(await ERC1xxxDelegate_MultisigRefund.deployed()).address,
			utils.prepareData(ERC1xxxDelegate_MultisigRefund, "initialize", [
		...
		Ident = await ERC1xxxDelegate_MultisigRefund.at(Proxy.address);
	});
