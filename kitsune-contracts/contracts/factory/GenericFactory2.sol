...
contract GenericFactory2 is CounterfactualFactory
{
	...
	function predictAddressWithCallback(bytes memory _code, bytes32 _salt, bytes memory _callback)
	public view returns(address)
	{
		return _predictAddress(_code, keccak256(abi.encodePacked(_salt, _callback)));
	}

	function createContractAndCallback(bytes memory _code, bytes32 _salt, bytes memory _callback)
	public returns(address)
	{
		address addr = createContract(_code, keccak256(abi.encodePacked(_salt, _callback)));
